...
export function For<T>(props: ForProps<T>): Node {
  ...
  // Dev-only: warn about duplicate keys once per component instance to avoid
  // flooding the console on lists reconciled per keystroke.
  let warnedDuplicateKey = false;

  const getKeys = (items: T[]): unknown[] => {
    const keys = new Array<unknown>(items.length);
    const seen = __DEV__ && !warnedDuplicateKey ? new Set<unknown>() : null;
    for (const [i, item] of items.entries()) {
      ...
      if (seen) {
        if (seen.has(key)) {
          ...
          warn(
            `[For] duplicate key ${typeof key === 'string' ? JSON.stringify(key) : String(key)} detected. Duplicate keys may cause rows to remount or lose state.`,
          );
        } else {
          seen.add(key);
        }

...
export function createResource<T>(
  ...
  const applyReady = (result: T): void => {
    value.value = result;
    state.value = 'ready';
    loading.value = false;
    error.value = null;
  ...
  const applyError = (raw: unknown): void => {
    error.value = raw instanceof Error ? raw : new Error(String(raw));
    state.value = 'errored';
    loading.value = false;
  };
  ...
  const actions: ResourceActions<T> = {
    mutate: (newValue: T) => applyReady(newValue),
    refetch: () => doFetch(),
  };

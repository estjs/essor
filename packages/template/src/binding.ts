...
function resolve(node: Element, prop: string): Strategy {
  switch (node.nodeName) {
    case 'INPUT':
      if (prop === 'checked')
        return (node as HTMLInputElement).type === 'radio' ? RADIO : CHECKBOX
...
export function bindElement(
  ...
  const cast = shouldCast
    ? (v: unknown) => applyModifiers(v, trim, toNum)
    : (v: unknown) => v
  ...
  if (ime && !lazy) {
    addEventListener(node, 'compositionstart', () => { composing = true })
